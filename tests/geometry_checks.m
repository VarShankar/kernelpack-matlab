...
tri = kp.geometry.triangulateClosedSurface(X);
assert(size(tri, 2) == 3 && all(tri(:) >= 1) && all(tri(:) <= size(X, 1)), ...
    'Closed-surface visualization triangulation returned invalid indices.');
