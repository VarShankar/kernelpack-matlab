classdef PUSLFDAdvectionDiffusionReactionSolver < handle
    %PUSLFDADVECTIONDIFFUSIONREACTIONSOLVER PU-SL transport + FD diffusion + explicit reaction.
    ...
    methods
        function init(obj, domain, xi_sl, xi_fd, dt, nu, direction, num_omp_threads)
            ...
            requireMatchingNodes(obj.physical_nodes, obj.diffusion.getOutputNodes(), ...
                'PUSLFDAdvectionDiffusionReactionSolver::init');
        ...
        function next_state = bdf1Step(obj, t_next, velocity, rk, reaction, forcing, NeuCoeffFunc, DirCoeffFunc, bc)
            assert(~isempty(obj.coeff_nm2), 'PUSLFDAdvectionDiffusionReactionSolver::bdf1Step requires setInitialState first.');
        ...
        function next_state = bdf2Step(obj, t_next, velocity, rk, reaction, forcing, NeuCoeffFunc, DirCoeffFunc, bc)
            assert(obj.completed_steps >= 1 && ~isempty(obj.coeff_nm1), ...
                'PUSLFDAdvectionDiffusionReactionSolver::bdf2Step requires one prior step.');
        ...
        function next_state = bdf3Step(obj, t_next, velocity, rk, reaction, forcing, NeuCoeffFunc, DirCoeffFunc, bc)
            assert(obj.completed_steps >= 2 && ~isempty(obj.coeff_n), ...
                'PUSLFDAdvectionDiffusionReactionSolver::bdf3Step requires two prior steps.');
            velocity_fn = velocity;
...
if num_steps <= 0
    error('kp:solvers:BadStepCount', 'PUSLFDAdvectionDiffusionReactionSolver::transportCoefficients requires a positive step count.');
end
original_dt = obj.dt;
